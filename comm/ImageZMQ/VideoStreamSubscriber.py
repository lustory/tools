...
class VideoStreamSubscriber:
    ...
    port：      传输端口
    req_rep: 是否采用request_reply模式，若为是，则接收到消息后须给返回值，否则
                不用返回值。
    dtype:      传输的数据类型是否为buffer，若为是则采用rec_jpg，否则为rec_image
    """


    def __init__(self, hostname, port, req_rep=False, dtype="buffer"):
        ...
        self._thread = threading.Thread(target=self._run, args=(req_rep,))
    ...
    def _run(self, req_rep):
        receiver = imagezmq.ImageHub("tcp://{}:{}".format(self.hostname, self.port), REQ_REP=req_rep)
